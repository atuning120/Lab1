...
static void SoftwareOfimatica(Ofimatica* of){
    ...
    while(true){
        ...
        if(opcion=="1"){
            of->agregarArchivo();
            cout<<"Se añadio un archivo\n";

        //Eliminar archivo
        }else if(opcion=="2"){
            of->eliminarArchivo();
            cout<<"Se elimino un archivo\n";


        //Mostrar archivos
        }else if(opcion=="3"){
            cout<<"Hay "+ to_string(of->getCant()) + " Archivos\n";
        //Salir
        }else if(opcion=="4"){
            return;
        }
...
//Funcion de la red social
static void RedSocial(vector<Usuario*> listaUsuarios,Usuario* u,Social* sos){
    string respuesta;
    while(true){
        cout<<"********Social*******\n1. Añadir amigo\n2. Eliminar Amigo\n3. Ver amigos\n4. Salir\nOpcion: ";
        cin>>respuesta;
        ...
        if(respuesta=="1"){
            ...
            cout<<"******Añadir amigo******\nNombre: ";
            cin>>nombre;
            sos->añadirAmigo(listaUsuarios,nombre,u);
        ...
        //Eliminar Amigo
            ...
            cout<<"******Eliminar Amigo*******\nNombre: ";
            cin>>nombre;
            sos->eliminarAmigo(nombre);

        //Ver amigos
        }else if(respuesta=="3"){
            cout<<"*******Todos los amigos*****\n";
            sos->verAmigos();

        //Salir de la red social
        }else{
            return;
        }
...
static void menu(vector<Usuario*> Usuarios,vector<Software*> Softwares,Usuario* u){
    ...
    while(option!="5"){
        ...
        //Softwares descargados 
            ...
            cout<<"Software: ";
            cin>>nameSoft;
            //Si el software seleccionado es:
            //Social
            Social* sos= dynamic_cast<Social*>(u->getSoftware(nameSoft));
            Ofimatica* ofima= dynamic_cast<Ofimatica*>(u->getSoftware(nameSoft));
            if(sos){
                RedSocial(Usuarios,u,sos);

            //Ofimatica
            }else if(ofima){
                SoftwareOfimatica(ofima);
            }
